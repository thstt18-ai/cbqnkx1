...
async function validateContract(address: string, config: any): Promise<boolean> {
  try {
    ...
    if (!rpcUrl) {
      console.error('RPC URL is not configured for validation. Check your bot configuration.');
      return false;
    }
    ...
    // Дополнительная проверка: пытаемся вызвать метод контракта
    const ABI = ['function EXECUTOR_ROLE() view returns (bytes32)'];
    const contract = new ethers.Contract(address, ABI, provider);
    
    try {
      await contract.EXECUTOR_ROLE();
      return true;
    } catch {
      console.warn(`⚠️ Contract at ${address} doesn't have EXECUTOR_ROLE method`);
      return false;
    }

  } catch (validationError: any) {
    console.error(`Error validating contract at ${address}: ${validationError.message}`);
    return false;
  }
}
