...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/wallet/balance", async (req, res) => {
    try {
      ...
      // Use walletAddress from config, or derive from private key
      ...
      // Get WETH balance (assuming WETH address is consistent or fetched)
      const wethAddress = POLYGON_TOKENS.WETH.address; // Assuming WETH address is available in POLYGON_TOKENS
      const wethContract = new ethers.Contract(wethAddress, usdcAbi, provider); // Reusing usdcAbi for ERC20 balanceOf
      const wethBalance = await wethContract.balanceOf(walletAddress);
      const wethFormatted = ethers.formatUnits(wethBalance, 18); // WETH has 18 decimals
